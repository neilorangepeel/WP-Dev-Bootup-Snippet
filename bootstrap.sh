...
# Auto-fix common wp-config.php mistakes (unquoted 256M / env type)
if ! php -l wp-config.php >/dev/null 2>&1; then
  ...
  # Quote bare M/G values, e.g. 256M -> '256M'
...
# Confirm WP loads (without plugins/themes)
...
wp option update blogdescription "Just another NOP site."
...
# Medium / Large: width-only (height 0 preserves aspect ratio)
wp option update medium_size_w 900
wp option update medium_size_h 0
# Medium Large (HiDPI candidate used in srcset)
wp option update medium_large_size_w 1536
wp option update medium_large_size_h 0
# Large ≈ typical content width
wp option update large_size_w 1400
...
# Create or reuse a Navigation entity titled 'Main Navigation', add Home + Blog
