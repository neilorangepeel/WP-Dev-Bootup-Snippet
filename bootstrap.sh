...
# If wp-config.php has a parse error (often unquoted 256M), try to auto-fix, then re-check
if ! php -l wp-config.php >/dev/null 2>&1; then
	echo "wp-config.php has a PHP syntax error; attempting auto-fix for memory constants…"
	cp wp-config.php "wp-config.php.bak.$(date +%s)"
	# Quote bare M values for memory constants
	perl -i -pe "s/define\(\s*'WP_MEMORY_LIMIT'\s*,\s*([0-9]+)M\s*\)/define('WP_MEMORY_LIMIT', '\$1M')/g" wp-config.php
	perl -i -pe "s/define\(\s*'WP_MAX_MEMORY_LIMIT'\s*,\s*([0-9]+)M\s*\)/define('WP_MAX_MEMORY_LIMIT', '\$1M')/g" wp-config.php
	php -l wp-config.php >/dev/null || { echo "Still seeing a syntax error in wp-config.php. Fix manually and re-run."; exit 1; }
fi
...
# String constant must be quoted in PHP -> no --raw (lets WP-CLI add quotes)
wp config set WP_MEMORY_LIMIT      256M        --type=constant        2>/dev/null || true
# Booleans/ints ok as raw
